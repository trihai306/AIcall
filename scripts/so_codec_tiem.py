...
Vòng thử này soi khâu CUỐI - codec trộn AIF1RX vào AIF1TX. Đã loại được
AudioTrack: phát ra loa ngoài rồi thu lại nghe sạch hoàn toàn.

    tuyến một  ->  AIF1TX1 Input 2 = AIF1RX1     (đang dùng)
    tuyến hai  ->  AIF1TX1 Input 2 = AIF1RX2     (thử kênh còn lại)
    tuyến ba   ->  đi ĐÚNG đường micro: AIF1RX1 -> ASRC1IN1L -> AIF1TX1 Input 1,
                   tức qua cả bộ đổi tần mà HAL dựng cho tuyến cuộc gọi

    python scripts/so_codec_tiem.py 0396130621
"""
...
def tuyen(so: int) -> list[tuple[str, str]]:
    ...
    if so == 1:
        cap += [("AIF1TX1 Input 2", "AIF1RX1"), ("AIF1TX2 Input 2", "AIF1RX1")]
    elif so == 2:
        cap += [("AIF1TX1 Input 2", "AIF1RX2"), ("AIF1TX2 Input 2", "AIF1RX2")]
    else:
        # Đúng chuỗi HAL dựng cho micro, chỉ thay nguồn ở đầu vào: thay vì lấy
        # LHPF1 (micro) thì lấy AIF1RX1 (tiếng AI). Nhờ vậy đi qua cả bộ đổi tần
        # ASRC mà tuyến cuộc gọi vốn có.
        cap += [("ASRC1IN1L Input 1", "AIF1RX1"),
                ("ASRC1IN1R Input 1", "AIF1RX1"),
...
async def chay(a) -> int:
    ...
    try:
        ...
        for so in (a.tuyen or [1, 2, 3]):
            ...
            await asyncio.sleep(0.6)
            ten = {1: "AIF1TX1 Input 2 <- AIF1RX1",
                   2: "AIF1TX1 Input 2 <- AIF1RX2",
                   3: "qua ASRC1IN1L, đúng đường micro"}[so]
            print(f"\n    --- tuyến {so}: {ten}")
        ...
        dat(("ASRC1IN1L Input 1", "LHPF1"), ("ASRC1IN1R Input 1", "LHPF2"))
