...
chọn thẳng được SIFS1.
    ...
    tuyến một  ->  NSRC1 = SIFS1, KHÔNG tiêm ở codec   (thẳng ABOX -> modem)
    tuyến hai  ->  NSRC1 = SIFS1 VÀ tiêm ở codec       (cả hai)
    tuyến ba   ->  NSRC1 = UAIF0, tiêm ở codec         (cách hiện tại, để đối chiếu)

    python scripts/so_abox_tiem.py 0396130621
"""
...
def tuyen(so: int) -> list[tuple[str, str]]:
    ...
    if so == 1:
        # Thẳng ABOX -> modem. Không bật chiều phát UAIF0, không đụng codec.
        cap += [("ABOX UAIF0 SPK", "RESERVED"), ("ABOX NSRC1", "SIFS1")]
    elif so == 2:
        cap += [("ABOX UAIF0 SPK", "SIFS1"), ("ABOX NSRC1", "SIFS1"),
                ("AIF1TX1 Input 2", "AIF1RX1"), ("AIF1TX2 Input 2", "AIF1RX1")]
    else:
        # Cách hiện tại, giữ nguyên để đối chiếu.
...
async def chay(a) -> int:
    ...
    try:
        ...
        for so in (a.tuyen or [1, 2, 3]):
            ...
            await asyncio.sleep(0.6)
            ten = {1: "NSRC1=SIFS1, KHÔNG qua codec",
                   2: "NSRC1=SIFS1 và có tiêm ở codec",
                   3: "NSRC1=UAIF0, tiêm ở codec (cách hiện tại)"}[so]
            print(f"\n    --- tuyến {so}: {ten}")
