...
nghe lại chính nó). Nhưng chưa hề kiểm `SIFS1` có chạy đúng tần số không.
    ...
    tuyến một  ->  SIFS1                      (đang dùng)
    tuyến hai  ->  SIFS0                      (tuyến gốc, AI sẽ nghe lại mình)
    tuyến ba   ->  SIFS1, hạ đường tiêm 6 dB  (phòng khi vỡ trong codec)

    python scripts/so_tuyen_tiem.py 0396130621
"""
...
def tuyen(so: int) -> list[tuple[str, str]]:
    """Bộ control cho từng tuyến. Luôn tắt hết đầu vào khác của mixer đường lên."""
    sifs = "SIFS0" if so == 2 else "SIFS1"
    vol = "26" if so == 3 else "32"          # 32 = 0dB, mỗi bước 1dB
    cap = [(f"ABOX SPUS OUT{i}", sifs) for i in range(6)]
    cap += [("ABOX UAIF0 SPK", sifs),
            ("AIF1TX1 Input 2", "AIF1RX1"), ("AIF1TX2 Input 2", "AIF1RX1"),
            ("AIF1TX1 Input 2 Volume", vol), ("AIF1TX2 Input 2 Volume", vol)]
    cap += [(f"AIF1TX{tx} Input {i}", "None") for tx in (1, 2) for i in (1, 3, 4)]
    return cap
...
async def chay(a) -> int:
    ...
    try:
        ...
        for so in (a.tuyen or [1, 2, 3]):
            ...
            await asyncio.sleep(0.6)
            print(f"\n    --- tuyến {so}: SPUS->{'SIFS0' if so == 2 else 'SIFS1'}"
                  f"{', hạ 6dB' if so == 3 else ''}")
        ...
        dat(("AIF1TX1 Input 2 Volume", "32"), ("AIF1TX2 Input 2 Volume", "32"))
